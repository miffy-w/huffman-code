...
const { readFile, writeFile } = node_fs_1.default.promises;
class CompressFile {
    constructor() {
        /** 编码表 */
        this.codingTable = new Map();
        /** 统计字符个数 */
        this.byteNumMap = new Map();
        /** 哈夫曼树 */
    ...
    compress(input, output) {
        /** 如果文件不存在 */
        const fileStat = node_fs_1.default.statSync(input);
    ...
    decompress(input, output) {
        ...
        */
        const readStream = node_fs_1.default.createReadStream(input);
        ...
        readStream.on('end', () => {
            ...
            const bitLength = bitSizeBuf.readUint32BE(0);
            try {
                ...
                debugger;
                const buffer = this.getOriginBuffer(decodingTable, zipBinary, bitLength);
                debugger;
    ...
    /** 根据解码表和压缩后的数据生成原始数据 */
    getOriginBuffer(decodingTable, buffer, bitLength) {
        let binaryStr = '';
    ...
    emitZipFile(compressedBuf, codingTable, output) {
        /** 使用 stream 存储 */
        const writeStream = node_fs_1.default.createWriteStream(output, { encoding: 'binary' });
    ...
    _createHuffmanTree(nodeArr) {
        while (nodeArr.length > 1) {
            /** 先从小到大排序 */
            this.sort(nodeArr);
            /** 节点权值两两相加，生成父节点 */
            const node1 = nodeArr.pop();
            const node2 = nodeArr.pop();
            /** 父节点没有内容，只有权值 */
            const parentNode = new TreeNode_1.TreeNode(node1.weight + node2.weight);
    ...
    createHuffmanTree(byteNumMap) {
        const nodeArray = [];
        for (let [byte, weight] of byteNumMap) {
            nodeArray.push(new TreeNode_1.TreeNode(weight, byte));
        }
        /** 开始生成哈夫曼树 */
        return this._createHuffmanTree(nodeArray);
    }
    _getCodingTable(node, bit, strBuilder = '', table) {
        if (node) {
            /** 把传入的 code 拼接成字符串 */
            strBuilder += bit;
